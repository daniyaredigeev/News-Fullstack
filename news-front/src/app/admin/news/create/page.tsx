...
import { apiCreateNews, apiGetCities, apiUploadImage } from '@/lib/api';
...
export default function CreateNewsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.title || !form.excerpt || !form.content || !form.cityId) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Ошибка при создании новости');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                value={form.excerpt}
                onChange={handleChange}
                placeholder="Краткое описание новости (для карточки на главной)"
                rows={2}
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {tags.map((tag) => (
              <span
                key={tag}
                className="bg-green-50 text-green-700 text-sm px-3 py-1 rounded-full flex items-center gap-1.5"
              >
                #{tag}
                <button
                  type="button"
          ...
          />
          <p className="text-xs text-gray-400 mt-1">Нажмите Enter или запятую для добавления</p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            />
            <div>
              <div className="font-medium text-gray-900 text-sm">Опубликовать сразу</div>
              <div className="text-xs text-gray-400">Если не выбрано, новость будет сохранена как черновик</div>
            </div>
          </label>
        </div>
        ...
        <div className="flex gap-3 pb-6">
          <button
            type="submit"
            disabled={submitting}
            className="bg-green-600 hover:bg-green-700 text-white font-semibold px-8 py-3 rounded-lg transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
          >
